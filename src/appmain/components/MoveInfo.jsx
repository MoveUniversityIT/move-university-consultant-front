...
const getPriceByDistance = (dokchaPrices, distance) => {
  const priceInfo = dokchaPrices?.find(
    (price) => distance >= price.minDistance && distance < price.maxDistance
  );
...
const MoveInfo = ({
  ...
  const [storageLoadRequestDate, setStorageLoadRequestDate] = useState(
    dayjs(new Date())
  );
  const [storageLoadRequestTime, setStorageLoadRequestTime] = useState(
    dayjs("08:00", "HH:mm")
  );

  const [storageUnloadRequestDate, setStorageUnloadRequestDate] = useState(
    dayjs(new Date())
  );
  const [storageUnloadRequestTime, setStorageUnloadRequestTime] = useState(
    dayjs("08:00", "HH:mm")
  );
  ...
  // 특이사항 목록
  const collapseSpecialItems = Object.values(
    consultantData?.specialItems || {}
  ).flat();
  ...
  const {
    isLoading: isDistanceData,
    data: roadDistanceData,
    isFetching: isDistanceFetching,
  } = useRoadDistance(locationInfo);

  const {
    data: dispatchAmount,
    isLoading: isDispatchAmount,
    error: dispatchError,
  } = useCalcConsultant(consultantDataForm, isFormValid, isDistanceFetching);
  ...
  const handleLocationChange =
    (setLocation, setShowList, locationType) => (e) => {
  ...
  const handleAddressSelect =
    (setLocation, setShowList, locationType) => (address) => {
      setLocation(address);
      setLocationSearch({ address, locationType });
      setShowList(false);
    };
    ...
    if (locationSearch?.locationType === "load") {
      if (locationList) {
        ...
        if (
          ...
          locationList.address[0]?.address?.address_name
        ) {
          handleLoadCoordinates({
            x: locationList.address[0]?.x,
            y: locationList.address[0]?.y,
          });
          handleAddressSelect(setLoadLocation, setShowLoadAddressList);
          setLoadCityCode(
            locationList.address[0]?.address?.b_code?.trim() || null
          );
        }
      } else {
  ...
  useEffect(() => {
    if (roadDistanceData && roadDistanceData.distance !== "undefined") {
      ...
      setDokchaPrice(
        getPriceByDistance(consultantData?.dokchaPrices, newDistance)
      );
    }
  }, [roadDistanceData]);
  ...
  const handleMoveTypeChange = (setter) => (value, option) => {
    ...
    if (value === '단순운송' || value === '일반이사') {
        Object.entries(updatedItems).forEach(([key, item]) => {
            if (itemMapping[item?.itemName] && validateItem(item)) {
                const count = item.itemCount;
                const newItemName = itemMapping[item.itemName];
                
                // 기존 포장됨 항목 찾기
                const existingKey = Object.keys(updatedItems).find(
                    k => updatedItems[k]?.itemName === newItemName && validateItem(updatedItems[k])
                );

                if (existingKey) {
                    // 기존 포장됨 항목이 있는 경우
                    const existingCount = updatedItems[existingKey].itemCount ?? 0;
        ...
        Object.entries(updatedItems).forEach(([key, item]) => {
            // itemCount가 0이고 neededCount가 있는 비정상 케이스 처리
            if (item?.itemCount === 0 && item?.neededCount > 0) {
                item.itemCount = item.neededCount;
            }

            if (reverseMapping[item?.itemName] && item?.neededCount > 0) {
                const originalItemName = reverseMapping[item.itemName];
                const countToRevert = item.neededCount;
                ...
                if (existingKey) {
                    ...
                    updatedItems[existingKey] = {
                        ...updatedItems[existingKey],
                        itemCount: existingCount + countToRevert
                    };
                } else {
                    updatedItems[originalItemName] = {
                        ...item,
                        itemName: originalItemName,
                        itemCount: countToRevert
                    };
                }

                // 포장됨 상태 업데이트
                const remainingCount = (item.itemCount ?? 0) - countToRevert;
                if (remainingCount > 0) {
                    updatedItems[key] = {
                        ...item,
                        itemCount: remainingCount
                    };
                    delete updatedItems[key].neededCount;
                } else {
                    delete updatedItems[key];
                }
            }
    ...
    const updatedSearchTerms = Object.values(updatedItems)
      .map((item) => {
        ...
        // 조건에 따라 tags 값 설정
        if (
          item?.requiredIsDisassembly === "Y" &&
          item?.requiredIsInstallation === "Y"
        ) {
          tags = "[분조]";
  ...
  const checkRequiredFields = () => {
    ...
    if (moveType?.value === "보관이사") {
      ...
      if (storageMoveType?.value === "포장이사" && items) {
        const hasPackingBox = Object.values(items).some(
          (item) => item.itemName === "박스(필요)"
        );
      ...
      if (_.isEmpty(storageLoadRequestDate))
        emptyFields.push("보관이사 상차 요청 날짜");
  ...
  const handleSave = () => {
    ...
    reservationMutate(
      { reservationData, hasAdminAccess },
      {
        onSuccess: (data) => {
          const successMessage = data?.reservationId
            ? "정상적으로 저장되었습니다."
            : "정상적으로 처리되지 않았습니다.";
          message.success({
        ...
        onError: (error) => {
          const errorMessage =
            error?.errorMessage || "상담 저장 중 에러가 발생했습니다";
          alert(errorMessage);
        },
      }
    );
  ...
  const handleSort = (key, orderKey) => {
    ...
    const updatedSearchTerm = sortedItems
      .map((item) => {
        ...
        const itemCount =
          item.itemCount && item.itemCount !== 1 ? item.itemCount : "";
  ...
  const handleSaveGongcha = () => {
    ...
    const transportHelperCount = dispatchAmount[0]?.helpers
      ? dispatchAmount[0].helpers.reduce((total, helper) => {
          ...
          return total;
        }, 0) - Math.min(loadTransCount, unloadTransCount)?.toLocaleString()
      : 0;

    const cleaningHelperCount = dispatchAmount[0]?.helpers
      ? dispatchAmount[0].helpers
          .reduce((total, helper) => {
    ...
    const head_count =
      Number(dispatchAmount[0]?.vehicleCount) +
      Number(transportHelperCount) +
      Number(cleaningHelperCount);
    ...
    saveGongchaMutate(gongchaData, {
      ...
      onError: (error) => {
        const errorMessage =
          error?.errorMessage || "공차등록 중 에러가 발생했습니다";
        message.error({
  ...
  useEffect(() => {
    if (reservationData) {
      ...
      setLoadMethod(
        reservationData?.loadMethod ?? { key: 1, value: "엘레베이터" }
      );
      ...
      setUnloadMethod(
        reservationData?.unloadMethod ?? { key: 1, value: "엘레베이터" }
      );
      ...
      setRequestTime(
        dayjs(reservationData.requestTime, "HH:mm") ?? dayjs("08:00", "HH:mm")
      );

      setStorageLoadRequestDate(
        dayjs(reservationData.storageLoadRequestDate) ?? dayjs(new Date())
      );
      setStorageLoadRequestTime(
        dayjs(reservationData.storageLoadRequestTime, "HH:mm") ??
          dayjs("08:00", "HH:mm")
      );

      setStorageUnloadRequestDate(
        dayjs(reservationData.storageUnloadRequestDate) ?? dayjs(new Date())
      );
      setStorageUnloadRequestTime(
        dayjs(reservationData.storageUnloadRequestTime, "HH:mm") ??
          dayjs("08:00", "HH:mm")
      );
      ...
      setVehicleTonnage(
        reservationData?.vehicleTonnage ?? { key: 1, value: "카고" }
      );
      setVehicleCount(reservationData?.vehicleCount ?? null);

      setHelpers(
        reservationData?.helpers ?? [
          { helperType: "TRANSPORT", peopleCount: 0 },
          { helperType: "PACKING_CLEANING", peopleCount: 0 },
        ]
      );
      ...
      setPaymentMethod(
        reservationData?.paymentMethod ?? { key: 1, value: "현금" }
      );
    ...
    if (checkRequiredFields()) {
      const formData = {
        ...
        storageLoadRequestDate:
          storageLoadRequestDate.format("YYYY-MM-DD") || null,
        storageLoadRequestTime: storageLoadRequestTime.format("HH:mm") || null,
        storageUnloadRequestDate:
          storageUnloadRequestDate.format("YYYY-MM-DD") || null,
        storageUnloadRequestTime:
          storageUnloadRequestTime.format("HH:mm") || null,
      ...
      if (moveType.value !== "포장이사") {
        const packingCleaningHelper = helpers.find(
          (helper) => helper.helperType === "PACKING_CLEANING"
        );
        const peopleCount = packingCleaningHelper
          ? packingCleaningHelper.peopleCount
          : 0;

        if (peopleCount > 0) {
          message.warning({
            content:
              "이사종류: 포장이사가 아닌경우 추가 이모 설정은 무시 후 계산됩니다.",
      ...
      if (moveType.value === "단순운송") {
        const transportHelper = helpers.find(
          (helper) => helper.helperType === "TRANSPORT"
        );
        const packingCleaningHelper = helpers.find(
          (helper) => helper.helperType === "PACKING_CLEANING"
        );
        const totalPeople =
          transportHelper?.peopleCount ??
          0 + packingCleaningHelper?.peopleCount ??
          0;

        if (totalPeople > 0) {
          message.warning({
            content:
              "이사종류: 단순운송인 경우 추가 인부, 추가 이모 설정은 무시 후 계산됩니다.",
            key: transportKey,
            duration: 3,
          });
        } else {
  ...
  return (
    <Card title="이사 정보" className="shadow-md rounded-md relative">
      <div
      ...
      <div className="flex gap-2 items-start mb-1">
              ...
              <Tooltip
                ...
                title={
                  <div className="text-sm">
                    ...
                    <strong>추가:</strong> 첫 대는 <strong>15박스</strong>, 이후{" "}
                    <strong>+12박스</strong>씩 추가. <br />
                    <span className="text-red-500">
                      ※ 박스 수 차이가 클 경우 금액이 달라질 수 있습니다.
                    </span>
                  </div>
                }
        ...
        <Form.Item className="flex-1 !mb-1">
          <label className="text-sm font-medium text-gray-700 mb-1 block">
            {moveType?.value !== "보관이사" ? "요청 시간:" : "상차 요청 시간:"}
          </label>
          <TimePicker
          ...
          <Form.Item className="flex-1 !mb-1">
            ...
            >
              {consultantData.moveTypes
                .filter(
                  (moveType) =>
                    moveType.moveTypeName !== "보관이사" &&
                    moveType.moveTypeName !== "단순운송"
                )
                .map((moveType) => (
                  <Option
                    key={moveType.moveTypeId}
      ...
      <div className="flex gap-2 items-start mb-1">
        ...
        <Form.Item className="flex flex-col !mb-1" style={{ flex: "0.7 1 0" }}>
          <div className="flex items-center mb-1">
            <Tooltip
              ...
              title={
                <>
                  <span className="font-bold">빈값:</span> 자동으로 배차 대수가
                  계산됩니다. <br />
                  <span className="font-bold">입력값:</span> 배차 대수를 강제로
                  지정할 수 있습니다.
                </>
              }
              overlayStyle={{ maxWidth: "400px" }}
        ...
        <Form.Item className="flex flex-col !mb-1" style={{ flex: "0.7 1 0" }}>
          ...
          <InputNumber
            ...
            max={10}
            value={
              helpers.find((item) => item.helperType === "TRANSPORT")
                ?.peopleCount || 0
            }
        ...
        <Form.Item className="!mb-1">
          <div className="flex items-center mb-1">
            <Tooltip
              ...
              title={
                <div className="text-sm">
                  <strong>혼자:</strong> 물품무게로 인해 추가되는 인부 설정
                  무시합니다.
                  <br />
                  <span className="text-red-500">
                    ※ 추가 인부, 추가 이모 설정에는 영향을 주지 않습니다.
                  </span>
                </div>
              }
              overlayStyle={{ maxWidth: "400px" }}
