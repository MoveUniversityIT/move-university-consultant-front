...
import "./SalesQna.css";
import {useDeleteSalesQna, useGetSalesQna, usePatchSalesQna, usePostSalesQna} from "@hook/useSalesQna";

const SalesQna = () => {
    const {data: salesQnaData, isLoading, refetch} = useGetSalesQna();
    const {mutate: postSalesQna} = usePostSalesQna();
    const {mutate: patchSalesQna} = usePatchSalesQna();
    const {mutate: postSalesQnaDelete} = useDeleteSalesQna();
    ...
    const mapToTreeData = (data) => {
        return data.map((node) => ({
            key: node.salesQnaId,
            title: node.salesQnaContent,
            children: node.children ? mapToTreeData(node.children) : [],
        }));
    };

    const treeData = salesQnaData ? mapToTreeData(salesQnaData) : [];
    ...
    return (
        <div className="bg-white shadow-lg rounded-lg p-6 h-full overflow-auto w-full">
            <h1 className="text-2xl font-bold text-blue-600 mb-4">영업 QnA</h1>
